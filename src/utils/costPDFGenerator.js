...
export const generateCostPDF = (items, regionKey = 'eastus', currencyKey = 'USD') => {
  try {
    ...
    doc.text("Arunim's IT Caffe — Azure Architecture Designer", margin, 20);
    
    doc.setFontSize(12);
    doc.setFont('helvetica', 'normal');
    doc.text('Cost Estimation & Compliance Report', margin, 30);
      doc.setFontSize(9);
    ...
    doc.text(`App Version: v2.1.0 | Author: Arunim Pandey`, margin, 45);
    ...
    doc.setFontSize(16);
    doc.setTextColor(0, 120, 212);
    doc.text('Architecture Compliance Summary', margin, 20);
    doc.setFontSize(11);
    doc.setTextColor(0, 0, 0);
    doc.text('This report includes a compliance summary based on Microsoft Azure best practices.', margin, 30);
    doc.setFontSize(10);
    doc.text('Compliance badge: ', margin, 40);
    let badge = '🟢 Compliant';
    if (costData.complianceScore < 90) badge = '🟡 Partial Compliance';
    if (costData.complianceScore < 70) badge = '🔴 Non-Compliant';
    doc.text(badge, margin + 40, 40);
    doc.setFontSize(9);
    doc.text(`Generated by Arunim's IT Caffe | v2.1.0 | ${new Date().toLocaleString()}`, margin, 285);
    ...
    doc.text('Generated by Arunim\'s IT Caffe — Azure Architecture Designer', margin, 290);
