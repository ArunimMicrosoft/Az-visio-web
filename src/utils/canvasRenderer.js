...
export const renderDiagramToCanvas = async (items, connections, boundaries = [], options = {}) => {
  ...
  ctx.fillText(`${environment} | v2.7.8 | ${dateStr}`, contentWidth - 20, HEADER_HEIGHT / 2);
  ...
  ctx.fillText(`Generated by Arunim's IT Caffe | Compliance: ${complianceText} | v2.7.8 | ${options.author || 'Arunim Pandey'} | ${options.timestamp ? new Date(options.timestamp).toLocaleString() : ''}`, contentWidth / 2, footerCenterY);
