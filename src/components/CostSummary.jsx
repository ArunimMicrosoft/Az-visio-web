...
const CostSummary = ({ items, onRegionChange, onCurrencyChange, useRealTimeAPI = false }) => {
  ...
  const [loadingRealTime, setLoadingRealTime] = useState(false);  useEffect(() => {
    let cancelled = false;
    ...
    if (!items || items.length === 0) {
      ...
      setLoadingRealTime(true);
        ...
        .then(realTimeCost => {
          ...
          if (!cancelled) {
            // If API returns $0, fall back to static pricing
            if (realTimeCost.totalMonthly === 0 || realTimeCost.pricedItemCount === 0) {
              ...
              console.warn('API Response:', realTimeCost);
              const staticCost = calculateCost(items, selectedRegion, selectedCurrency);
              setCostData({
                ...staticCost,
                dataSource: 'Static Estimates (API returned $0)'
              });
              ...
              breakdown: realTimeCost.serviceBreakdown.map(s => ({
                ...
                retiredInfo: s.retiredInfo || null,                isNotMapped: s.isNotMapped || false,
              ...
              dataSource: 'Azure Retail Prices API (Official)',              pricedItemCount: realTimeCost.pricedItemCount || 0,
        ...
        .catch(error => {
          if (!cancelled) {
            ...
            setCostData({
              ...cost,
              dataSource: 'Static Estimates (API Error)'
            });
            setLoadingRealTime(false);
          }
        });
        ...
        <div className="cost-breakdown">
          ...
          {useRealTimePricing && (costData.retiredItemCount > 0 || costData.notMappedItemCount > 0) && (
            <div className="pricing-status-summary">
              ...
              </div>              <div className="status-summary-items">
                          ...
                          <div className="warning-body">
                            ...
                            <div className="warning-details">
                              <span className="warning-detail">
                                <strong>Status:</strong> {item.retiredInfo.status}
                              </span>
                              <span className="warning-detail">
                                <strong>Date:</strong> {item.retiredInfo.retiredDate}
                              </span>
                              <span className="warning-detail">
                                <strong>Replacement:</strong> {item.retiredInfo.replacement}
                              </span>
                            </div>
                          </div>
                        </div>
          ...
          </table><div className="cost-details-note">
            <small>
